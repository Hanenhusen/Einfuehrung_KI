...
for step in bar:
  # Wir erzeugen wieder die Prädiktion (Vorhergesagte Wahrscheinlichkeit)
  prediction = w[0] + w[1] * X + w[2] * X**2 + w[3] * X**3

  # Wir berechnen den LogCosh Loss
  logcosh = torch.mean(torch.log(torch.cosh(prediction - y)))

  bar.set_description(f"logcosh={logcosh.item():2f}")
  
  logcosh.backward()

  w.data = w.data - lernrate * w.grad 
  w.grad.data.zero_()
...
plt.scatter(X, y)
x_for_plot = torch.linspace(0, 5, 100)
y_for_plot = w[0] + w[1] * x_for_plot + w[2] * x_for_plot**2 + w[3] * x_for_plot**3
